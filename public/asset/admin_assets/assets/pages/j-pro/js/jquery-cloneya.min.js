    ...
    b.prototype = {
        init: function () {
            ...
            b.$elem.addClass(c + "-wrap"), b.clones.addClass(c), b.clones.data("initialCount", b.clones.length), b.$elem.on("click." + c, b.config.cloneThis + ">" + b.config.cloneButton, function (d) {
                ...
                var f = e.closest(b.elem)
                    .closestChild(b.config.cloneThis)
                    .length;
                f > b.config.minimum ? (b.$elem.triggerAll("clone_before_delete before_delete." + c, [e, f]), b.$elem.triggerHandler("remove." + c, [e]), b.$elem.triggerAll("clone_after_delete after_delete." + c)) : (b.$elem.triggerHandler("minimum." + c, b.config.minimum, [e]), e.find("input, textarea, select")
                    .each(function () {
                        b._clearForm(a(this))
                    }))
        ...
        _cloneAndAppend: function (a) {
            var b = a.closest(this.elem)
                .closestChild(this.config.cloneThis)
                .length;
            if (b < this.config.maximum) {
                this.$elem.triggerAll("clone_before_clone before_clone." + c, [a]);
                var d = this._cloneItem(a);
                this.$elem.triggerAll("clone_after_clone after_clone." + c, [a, d]), this.clones.add(d), this.$elem.triggerAll("clone_before_append before_append." + c, [a, d]), "after" !== this.config.clonePosition ? a.before(d) : a.after(d), this.config.ignore && d.find(this.config.ignore)
                    .remove(), this._redoIDs(), this.$elem.triggerAll("clone_after_append after_append." + c, [a, d])
        ...
        _cloneItem: function (b) {
            ...
            return d.config.preserveChildCount !== !1 && e.find("." + c + "-wrap")
                .each(function () {
                    var b = a(this)
                        .closestChild("." + c),
        ...
        _redoIDs: function () {
            ...
            if (b.config.serializeID === !0) {
                var c = b.$elem.find(b.config.cloneThis)
                    .first()
                    .attr("id");
                b.$elem.find(b.config.cloneThis)
                    .each(function (d) {
                        var e;
                        ...
                        a(this).find("*").each(function () {
                                ...
                                if (f = a(this).attr("name")) {
                                    var c = f.match(b.regex);
                                    c && 3 === c.length ? (g = f.replace(/\d+$/, "") + e, a(this).attr("name", g)) : (g = f + e, a(this).attr("name", g))
                                }
        ...
        if ("string" == typeof d && "_" !== d[0] && "init" !== d) {
            if (0 === Array.prototype.slice.call(e, 1)
                .length && -1 !== a.inArray(d, a.fn[c].getters)) {
